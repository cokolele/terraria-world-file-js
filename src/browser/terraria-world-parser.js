...
class terrariaWorldParser extends terrariaFileParser
{
	...
	parse(sections = ["fileFormatHeader", "header", "worldTiles", "chestsData", "signsData", "npcsData", "tileEntities", "pressurePlates", "townManager"])
	{
		let data = {};

		try
		{
			if (sections.includes("fileFormatHeader"))
				data.fileFormatHeader = this.LoadFileFormatHeader();
			else //section pointers must be loaded to continue
				this.LoadFileFormatHeader();
			if (sections.includes("header"))
				data.header = this.LoadHeader();
			else //map dimensions must be loaded to continue
				this.LoadHeader(true);
			if (sections.includes("worldTiles"))
				data.worldTiles = this.LoadWorldTiles();
			if (sections.includes("chestsData"))
				data.chestsData = this.LoadChests();
			if (sections.includes("signsData"))
				data.signsData = this.LoadSigns();
			if (sections.includes("npcsData"))
				data.npcsData = this.LoadNPCs();
			if (sections.includes("tileEntities"))
				data.tileEntities = this.LoadTileEntities();
			if (sections.includes("pressurePlates"))
				data.pressurePlates = this.LoadWeightedPressurePlates();
			if (sections.includes("townManager"))
	...
	ParseTileData()
	{
		...
		if ((flags1 & 2) == 2)
		{
			...
			else tile.blockId = this.ReadUInt8();
